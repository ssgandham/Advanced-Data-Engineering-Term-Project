...
public class Online_Test {
    ...
    public void BetweenNess_Centrality_Score(LinkedList<String> Distinct_nodes, LinkedList<String> source_vertex,
            LinkedList<String> target_vertex, LinkedList<Double> Edge_Weight) {
        ...
        Transformer wtTransformer = new Transformer() {
            public Double transform(MyLink link) {
                return link.weight;
            }
        };
        edu.uci.ics.jung.algorithms.scoring.BetweennessCentrality<MyNode, MyLink> BC1 =
                new edu.uci.ics.jung.algorithms.scoring.BetweennessCentrality<MyNode, MyLink>(g, wtTransformer);
        //Calculating Betweenness Centrality score of nodes
        for (int i = 0; i < Graph_Nodes_Only.size(); i++) {
            System.out.println("Graph Node " + Graph_Nodes_Only.get(i)
                    + " Betweenness Centrality +BC1.getVertexScore(Graph_Nodes_Only.get(i)))");
        }
